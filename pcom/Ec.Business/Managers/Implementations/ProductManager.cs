...
namespace Ec.Business.Managers.Implementations
{
	public class ProductManager : IProductManager
	{
		...
		public async Task<List<BrandDto>> GetBrand(string dynamicSearchingQuery)
        {
            var result = _mapper.Map<List<BrandDto>>(await _productRepository.GetBrandForSearchingProduct(dynamicSearchingQuery)); 
            return result.ToList();
        }

        public async Task<List<Author>> GetAuthor(string dynamicSearchingQuery)
        {
            var result = await _productRepository.GetAuthorListForSearchingProduct(dynamicSearchingQuery);
            return result.ToList();
        }
