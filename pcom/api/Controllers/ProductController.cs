...
namespace Ec.Api.Controllers
{
	...
	public class ProductController : ControllerBase
	{
		...
		[HttpGet("GetBrandForSearchingProduct")]
		[ProducesResponseType(typeof(IEnumerable<BrandDto>), 200)]
		[ProducesResponseType(404)]
		public async Task<IActionResult> GetBrandForSearchingProduct()
		{
			//
			string dynamicSearchingQuery = string.Empty;
			return Ok(await _productManager.GetBrand(dynamicSearchingQuery));
		}

		[HttpGet("GetAuthourForSearchingProduct")]
		[ProducesResponseType(typeof(IEnumerable<Author>), 200)]
		[ProducesResponseType(404)]
		public async Task<IActionResult> GetAuthourForSearchingProduct()
		{
			//
			string dynamicSearchingQuery = string.Empty;
			return Ok(await _productManager.GetAuthor(dynamicSearchingQuery));
		}
